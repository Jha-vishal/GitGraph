...
const endYear = 2022;
const repoPath = path.join(__dirname, 'weekend_git_graph'); // Path to the local repository
const commitMessage = 'Weekend contribution';
const commitsPerWeekend = 90;
...
// Start creating weekend commits
...
while (currentDate <= endDate) {
  ...
  if (currentDate.getDay() === 6 || currentDate.getDay() === 0) {
    for (let commit = 1; commit <= commitsPerWeekend; commit++) {
      // Modify the dummy file
      fs.appendFileSync(filePath, `Weekend commit on ${currentDate.toISOString()} - ${commit}\n`);
...
console.log('Weekend Git graph contributions have been created!');
